function parameterAll=getFeatureParameter(time,dirFile)
...
dirData=strcat(dirFile,'\allData');
% disp('please choose the save document')
dirSave='C:\Users\Administrator\Desktop';
% nameList=dir(dirData);
dataAll.dataInfo=[];
dataAll.correctionList=[];
dataAll.dataList=[];
threShold=0.15*30;
% if numel(time)~=size(nameList,1)-2
%     disp('Wrong time numble')
%     return
% end
featureParaNum=0;
serialNum=[];
...
for iCell=1:size(allData.dataInfo,2)
    ...
    featurePara{featureParaNum}.time=0;
...
dataAll.dataInfo=[dataAll.dataInfo,allData.dataInfo];
dataAll.correctionList(end+1:end+size(allData.correctionList,1),1:size(allData.correctionList,2))=allData.correctionList;
for iList=1:numel(allData.dataList)
    iDataList(iList,:)=allData.dataList{iList};
end
iDataList(:,3)=i;
dataAll.dataList=[dataAll.dataList;iDataList];
iDataList=[];
% end
for iCell=1:size(dataAll.dataInfo,2)
    cellInfo=dataAll.dataInfo{iCell};
...
paraSheet=zeros(size(dataAll.dataInfo,2),paraNum);
