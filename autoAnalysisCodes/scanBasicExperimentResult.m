function dirAllFile=scanBasicExperimentResult(dirAllFile)
clc
disp('Please input the experiment tpye number:')
disp('1.flowCell')
disp('2.montage')
disp('3.agarose longTime')
disp('4.simple mask meanIntensity')
trackingCase=input('____:');
switch trackingCase 
    case 1
        trackingCase='flowCell';
    case 2
        trackingCase='montage';
    case 3
        trackingCase='agarose';
    case 4 
        trackingCase='simpleMask';
    case 5
        trackingCase='montageJoint';
end
% load([dirAllFile,'\mip.mat']);
% load([dirAllFile,'\LumencorField.mat']);
% mip.Calib.LumencorVioletField=LumencorField.VioletField;
% mip.Calib.LumencorBlueField=LumencorField.BlueField;
% mip.Calib.LumencorCyanField=LumencorField.CyanField;
% mip.Calib.LumencorTealField=LumencorField.TealField;
% mip.Calib.LumencorGreenField=LumencorField.GreenField;
% mip.Calib.LumencorRedField=LumencorField.RedField;
% save([dirAllFile,'\mip.mat'],'mip');
if strcmp(trackingCase,'montageJoint')
    montageImageInitialized(dirAllFile,mip);
    return
end
allNameList=dir(dirAllFile);
% 初始化荧光图像，将数值图像转化为浓度量 存储在对应的荧光蛋白new中
% fluoImageInitialized(dirAllFile,mip);
for iField=1:numel(allNameList)-2
    ...
    if strcmp(trackingCase,'flowCell')
        ...
        %         bacNum=getBasicFigure([],dirFile,mip,fieldNum);
        bacNum=getBasicFigure([],dirFile,fieldNum);
        disp('treeProcessing')
        bioTree=batchTreeTrackingJob(dirFile);
%         bioTree{1}.bestPositionAccumulation=zeros(numel(bioTree),2);
...
%         end
        ...
        while n==0
            try
                [bioTree,~,~,emptyNode]=myBiograph_new2(bioTree);
                n=1;
            catch err
                bioTree=bioTreeCut(bioTree,numel(bioTree)-50);
            end
        end
    ...
    %% agarose bottom
    if strcmp(trackingCase,'agarose')
        dirImage=[dirFile,'\Tracking'];
        nameList=dir(dirImage);
        disp('loading')
        n=0;
...
%                 maskImage=imerode(maskImage,ones(4));
                maskImage=imerode(maskImage,ones(2));%ys 明场相关图像改为ones(2),否则腐蚀太过
                maskImage=bwmorph(maskImage,'open');%ys 先腐蚀后膨胀，
                maskImages(:,:,2*n-1)=maskImage;
                maskImages(:,:,2*n)=maskImage;
            end
        end
        %         bacNum=getBasicFigure(maskImages,dirFile,mip,fieldNum);
        bacNum=getBasicFigure(maskImages,dirFile,fieldNum);
...
%         [maskImages,bestPositionAccumulation]=fluoImageCorrection(maskImages);
...
%         bioTree{1}.bestPositionAccumulation=bestPositionAccumulation;
    ...
    %% montage analysis
    if strcmp(trackingCase,'montage')
        dirImage=[dirFile,'\Tracking'];
        nameList=dir(dirImage);
        disp('loading')
        n=0;
    ...
    %% montage analysis
    if strcmp(trackingCase,'simpleMask')
        dirImage=[dirFile,'\Tracking'];
        nameList=dir(dirImage);
        disp('loading')
        n=0;
...
%         bacNum=getBasicFigure(maskImages,dirFile,mip,fieldNum);
              bacNum=getBasicFigure(maskImages,dirFile,fieldNum);
        clear maskImages
        createAllData_brieflyMask(dirFile,mip);
        continue
    end
    %%
    % bioTreeMeasure
%     load([dirFile,'\bioTree.mat'])
%     bioTree{1}.mip=mip;
...
%     save([dirFile,'\bioTree.mat'],'bioTree','-v7.3')
    ...
    if branchList(1,1)==0
        ...
        [ bioTree ] = bioTreeAllInfoGet( bioTree,dirFile);
        tree=smallTreeStructureSeperate(bioTree);
        tree=treeSizeAndGenerationMarker(tree,bioTree{1}.bioTreeTimer(end));
        save([dirFile,'\','bacInfo&tree'],'bioTree','tree','bacInfo','-v7.3');
        createAllData(dirFile);
        for iTree=1:numel(tree)
            if tree(iTree).leafNum>=3% originla 10
...
fileNameIndex=find(dirAllFile=='\');
fileName=dirAllFile(1:fileNameIndex(end)-1);
preName=dirAllFile(fileNameIndex(end)+1:end);
cd(fileName)
try
    eval(['!rename ',preName,[' ',preName,'_analysed']])
end
% movefile(dirAllFile,[dirAllFile,'_analysed'])
dirAllFile=[dirAllFile,'_analysed'];
end
...
if min(imageSize(1:2))>=1500;
    imageStackNew=imageStack(1024-150:1024+150,1024-150:1024+150,:);
else
...
mkdir(dirImage)
nameList=dir(dirTracking);
n=0;
j=0;
for i=2:numel(nameList)-2
    n=n+1;
    load([dirTracking,'\',nameList(i+2).name]);
    maskImages(:,:,n)=imageTracking;
    if n==200
        j=j+1;
        save([dirImage,'\',num2str(j,'%05.f')],'maskImages');
        n=0;
        clear maskImages
    end
end
...
allTreeSave(bioTree,strcat(dirResultSave,'\allTree'));
end
%%
