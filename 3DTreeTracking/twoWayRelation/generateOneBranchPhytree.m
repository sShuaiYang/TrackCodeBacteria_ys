function [linkMatrix,centroidInfo,leafList,leafNum,ageAll,nodeList]=generateOneBranchPhytree(bioTree,iBra,ibranch,type)
...
if strcmp(type,'normal')
    ...
    allRoot(:,3)=1;
    allRoot(:,4)=-1;
    allRoot(allRoot(:,1)>numel(bioTree),:)=[];
    ...
    while  ~all(traceMatrix{i}(:,3)==0)
        ...
        for iNum=1:size(traceMatrix{i},1)
            if traceMatrix{i}(iNum,3)==1 && traceMatrix{i}(iNum,1)>numel(bioTree)
                traceMatrix{i}(iNum,3)=0;
            end
