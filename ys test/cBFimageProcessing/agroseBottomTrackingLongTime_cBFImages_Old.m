function  agroseBottomTrackingLongTime_cBFImages_Old()
%移植longtimeProtocal的code进行处理
dirAllFile='\\192.168.1.14\e\2020-04-01 PAO1_IP32_100x Temp30 ys';
allNameList=dir(dirAllFile);
for iField = 2:64%numel(allNameList)-2
    t0=clock;
    if ~(strcmp(allNameList(iField+2).name(1:5),'field') ) && strcmp(allNameList(iField+2).name(end-2:end),'mat')
        continue
    end
%     clc
    fieldNum=str2double(allNameList(iField+2).name(end-3:end)); 
...
%     catch 
%         warning('Problem using the function agaroseBottomAutoSeg.Skipped.');
%     end
    %     n=0;
    %     while n==0
    %         try
    ...
    try
        ...
        warning('Problem using the function myBiograph_new2. Skipped.');        
    end
    ...
    disp('Try Cut Num and Two point aging')
    tic;
    try
    ...
    save([dirFile,'\','bacInfo&tree'],'bioTree','tree','growthData','-v7.3');

    disp(['共耗时:',num2str(etime(clock,t0)),'秒']);
end
