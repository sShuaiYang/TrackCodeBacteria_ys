function resultAll=newNetworkForF7(bioTree,bacteriaFrameInfo)
% function resultAll=newNetworkForF7(clusterTree,generateRelation,branchNum)
% F7分析专用
...
for iframe=1:numel(bioTree)-1
    for iLeaf=1:numel(bioTree{iframe}.leavies)
        leafAll=[leafAll;iframe,iLeaf,-1,1,bioTree{iframe}.leavies{iLeaf}.branchIndex,bioTree{iframe}.leavies{iLeaf}.leafMeasurment.Centroid(1),bioTree{iframe}.leavies{iLeaf}.leafMeasurment.Centroid(2)];
    end
end
...
for iframe=frameStep:frameStep:numel(clusterTree)
    ...
    % 第二步，建立新的连接
    newMatrix=updataNewMatrix(newMatrix,clusterTree(iframe).currentLink,p);
    preMatrix=newMatrix;
    if mod(iframe,step)==0 || iframe==numel(clusterTree)
       n=n+1;
       result(n).frameNum=iframe;
       result(n).finalMatrix=logical(newMatrix-full(clusterTree(iframe).branchMatrix));
    end
end
end
...
for i=1:size(newLink.info1,1)
    preMatrix(newLink.info1(i,1),:)=0;
    for iLink=1:numel(newLink.info2{i})
        preMatrix(newLink.info1(i,1),:)=preMatrix(newLink.info1(i,1),:) | preMatrix(newLink.info2{i}(iLink),:);
    end
end
newMatrix(matchOrder,:)=preMatrix(preMatch,:);
end
function newMatrix=updataNewMatrix(newMatrix,currentLink,p)
