function clusterTree=accumulateNetwork(bacteriaFrameInfo,bioTree,step)
lengthInfo=[];
...
for iFrame=1:size(bacteriaFrameInfo,2)
    ...
    %     D=(D<=(averageLength*ratio));
    D=(D<=(averageLength*1.2));    %%%%%%1.2 distance!!!!
    for i=1:size(D,1)
        D(i,i)=0;
    end
        ...
        if bacteriaInfo(3)~=0
            ...
            if bioTree{nodeInfo(1)}.node{nodeInfo(2)}.In{1}.isNode==0
                ...
                if preRoot(1)==iFrame
                    ...
                    preNodeSize=size(bioTree{nodeInfo(1)}.node{nodeInfo(2)}.In,2);
                    preParent=[];
                    ...
                    for iIn=1:preNodeSize-1
                        if bioTree{nodeInfo(1)}.node{nodeInfo(2)}.In{iIn+1}.isNode==0
                            preRoot=bioTree{nodeInfo(1)}.node{nodeInfo(2)}.In{iIn+1}.rootInfo;
                            preRoot=[preRoot,0];
                        ...
                        if bioTree{nodeInfo(1)}.node{nodeInfo(2)}.In{iIn+1}.isNode==1
                            preNode=bioTree{nodeInfo(1)}.node{nodeInfo(2)}.In{iIn+1}.nodeInfo;
                        ...
                        if ~isempty(ibacteria)
                            newClusterTree.distMatrix(ibacteria,:)=[];
                            newClusterTree.distMatrix(:,ibacteria)=[];
                            newClusterTree.bacteriaList(ibacteria,:)=[];
                            preTree.distMatrix(ibacteria,:)=[];
                            preTree.distMatrix(:,ibacteria)=[];
                            preTree.bacteriaList(ibacteria,:)=[];
                            correspondList(correspondList>ibacteria)=correspondList(correspondList>ibacteria)-1;
                        end
                    end
                    continue
                end
