%% past code
function [afterProcessingImages,imageProcessingInfo]=myImageProcessing_cBF(beforeProcessingImages,picType) %this function can segreate orignal images as you want and returen the mask images
...
% 4. no need to crop
cropInfo=true(size(beforeProcessingImages,1),size(beforeProcessingImages,2));
gaussianFilterSigma=2.6;%ys 将fspecial修改为imgaussfilt使用，sigma=2 适用于大多数的图像
%ys sigma=3 主要用于平滑cBF图像中的噪点100x
...
if strcmp(picType,'w')
    ...
    grayThresh=65;
    areaThreshold=220;  % proper 60, overlap 60+40
    maxIntensity=220;
    %     grayThresh=90;
...
parfor iframe=1:size(beforeProcessingImages,3)
    afterProcessingImages(:,:,iframe)=imgaussfilt(beforeProcessingImages(:,:,iframe),gaussianFilterSigma);
    ...
    afterProcessingImages(:,:,iframe)=imgaussfilt(afterProcessingImages(:,:,iframe),gaussianFilterSigma);
    %     thresholdImage=graythresh(afterProcessingImages(:,:,iframe)); %Otu thresholding images
    %     afterProcessingImages(:,:,iframe)=im2bw(afterProcessingImages(:,:,iframe),grayThresh/255);%1.2 for the image with few particle
%         afterProcessingImages(:,:,iframe)=imbinarize(beforeProcessingImages(:,:,iframe),180/255);%1.2 for the image with few particle
    autoGrayThresh=graythresh(afterProcessingImages(:,:,iframe)); %Auto thresholding images
    afterProcessingImages(:,:,iframe)=imbinarize(afterProcessingImages(:,:,iframe),autoGrayThresh/0.7);
