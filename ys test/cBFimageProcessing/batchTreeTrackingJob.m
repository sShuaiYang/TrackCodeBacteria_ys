function [bioTree,dirFile]=batchTreeTrackingJob(picType,~)
% dirFile=superBatchConvert(picType);
% dirImage=strcat(dirFile,'\tiff2matlab');   %put the folder of the save results
dirFile='C:\Users\XJY\Desktop\test-ys20191202';
dirImage='C:\Users\XJY\Desktop\test-ys20191202\BF stack';
...
tic;
% disp(strcat('1.load:',fileName,'...'));
disp(strcat('1.load:',fileName,'...'));
% imageStack=myLoad(i,fileName,dirResultSave);
imageStack=import_tiff_stack( strcat(dirImage,'\',nameList(i+2).name) );
toc;
...
[maskImages,imageProcessingInfo]=myImageProcessing(imageStack,'w');
