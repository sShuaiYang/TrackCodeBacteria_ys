function result=newNetworkDirectLink(bioTree,bacteriaFrameInfo)
% function result=newNetworkDirectLink(clusterTree,generateRelation,branchNum)
...
for iframe=frameStep:frameStep:numel(clusterTree)
    ...
    if iframe==frameStep
        newMatrix=false(size(clusterTree(iframe).nodeInfo,1),size(clusterTree(iframe).nodeInfo,1));
    else
...
%     newMatrix=newMatrix | full(clusterTree(iframe).branchMatrix);
    ...
    if mod(iframe,step)==0 || iframe==numel(clusterTree)
       ...
       result(n).finalMatrix=sparse(logical(newMatrix));
    end
end
end
...
newMatrix=false(size(newNode,1),size(newNode,1));
...
for iLink=1:size(currentLink,1)
    addMatrix(currentLink(iLink,2),currentLink(iLink,3))=1;
    addMatrix(currentLink(iLink,3),currentLink(iLink,2))=1;
end
